...
export class WizardNavigationService implements OnDestroy {
    ...
    constructor(
        ...
        this.pagesResetSubscription = this.wizardPageCollection.pagesReset.subscribe(() => {
   ...
   * @memberof WizardNavigationService
   */
    private _currentChanged = new Subject<WizardPageComponent>();

    /**
     * An Observable that is predominantly used amongst the subcomponents and services
     * of the wizard. It is recommended that users listen to the WizardPageComponent.onLoad
     * (WizardPageComponentPageOnLoad) output instead of this Observable.
     *
     * @memberof WizardNavigationService
     */
    public get currentPageChanged(): Observable<WizardPageComponent> {
     ...
     /**
     * A Boolean flag used by the WizardPageComponent to avoid a race condition when pages are
    ...
    public get currentPageIsFirst(): boolean {
        return this.wizardPageCollection.firstPage === this.currentPage;
    }
    ...
    public get currentPageIsLast(): boolean {
        return this.wizardPageCollection.lastPage === this.currentPage;
    }
   ...
   */
    ...
    /**
     * Returns the WizardPageComponent object of the current page or null.
     *
     * @memberof WizardNavigationService
     */
   ...
   */
    public forceNext(): void {
        const currentPage: WizardPageComponent = this.currentPage;
        const nextPage: WizardPageComponent = this.wizardPageCollection.getNextPage(currentPage);
        ...
        if (!currentPage.completed) {
            ...
            this.wizardPageCollection.commitPage(currentPage);
        }
    ...
    public previous(): void {
        let previousPage: WizardPageComponent;

        if (this.currentPageIsFirst || this.wizardStopNavigation) {
            return;
        }

        previousPage = this.wizardPageCollection.getPreviousPage(this.currentPage);
   ...
   * page.
   *
   * Takes two parameters. The first one must be either the WizardPageComponent object or the ID of the
   * WizardPageComponent object that you want to make the current page.
   *
   ...
   *
   * To avoid checks on navigation, use WizardPageComponent.makeCurrent() instead.
   *
   * @memberof WizardNavigationService
   */
    public goTo(pageToGoToOrId: any, lazyComplete: boolean = false) {
        let pageToGoTo: WizardPageComponent;
        let currentPage: WizardPageComponent;
        let myPages: WizardPageCollectionService;
        let pagesToCheck: WizardPageComponent[];
        let okayToMove: boolean = true;
        let goingForward: boolean;
        let currentPageIndex: number;
        let goToPageIndex: number;

        myPages = this.wizardPageCollection;
        ...
        pagesToCheck.forEach((page: WizardPageComponent) => {
            if (page !== pageToGoTo) {
            page.completed = true;
            }
        });
    ...
    /**
     * Accepts a range of WizardPageComponent objects as a parameter. Performs the work of checking
     * those objects to determine if navigation can be accomplished.
     *
     * @memberof WizardNavigationService
     */
    public canGoTo(pagesToCheck: WizardPageComponent[]): boolean {
        let okayToMove = true;
        const myPages = this.wizardPageCollection;
    ...
    public setLastEnabledPageCurrent(): void {
        const allPages: WizardPageComponent[] = this.wizardPageCollection.pagesAsArray;
        let lastCompletedPageIndex: number = null;

        allPages.forEach((page: WizardPageComponent, index: number) => {
            if (page.completed) {
    ...
    public setFirstPageCurrent(): void {
        this.currentPage = this.wizardPageCollection.pagesAsArray[0];
    }
    ...
    public updateNavigation(): void {
        let toSetCurrent: WizardPageComponent;
        let currentPageRemoved: boolean;

        this.wizardPageCollection.updateCompletedStates();

        currentPageRemoved = this.wizardPageCollection.pagesAsArray.indexOf(this.currentPage) < 0;
        if (currentPageRemoved) {
            toSetCurrent = this.wizardPageCollection.findFirstIncompletePage();
