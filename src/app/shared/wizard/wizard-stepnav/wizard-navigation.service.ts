...
export class WizardNavigationService implements OnDestroy {
     ...
     * of the Wizard.forceForward (WizardForceForwardNavigation) input. When true,
    ...
    /**
     * Accepts a WizardPageComponent object, since that object to be the current/active
     * page in the wizard, and emits the WizardPageComponent.onLoad (WizardPageComponentOnLoad)
     * event for that page.
     *
     * Note that all of this work is bypassed if the WizardPageComponenta object is already
     * the current page.
     *
     * @memberof WizardNavigationService
     */
   ...
   */
    ...
    /**
     * A boolean flag shared across the Wizard subcomponents that follows the value
     * of the Wizard.stopNavigation (WizardPreventNavigation) input. When true, all
    ...
    constructor(
        private pageService: WizardPageCollectionService,
        ...
        this.pagesResetSubscription = this.pageService.pagesReset.subscribe(() => {
    ...
    public get currentPageIsFirst(): boolean {
        return this.pageService.firstPage === this.currentPage;
    }
    ...
    public get currentPageIsLast(): boolean {
        return this.pageService.lastPage === this.currentPage;
    }
    ...
    public setFirstPageCurrent(): void {
        this.currentPage = this.pageService.pagesAsArray[0];
    }
    ...
    public canGoTo(pagesToCheck: WizardPageComponent[]): boolean {
        let okayToMove = true;
        const myPages = this.pageService;
    ...
    public updateNavigation(): void {
        ...
        this.pageService.updateCompletedStates();

        currentPageRemoved = this.pageService.pagesAsArray.indexOf(this.currentPage) < 0;
        if (currentPageRemoved) {
            toSetCurrent = this.pageService.findFirstIncompletePage();
    ...
    public forceNext(): void {
        ...
        const nextPage: WizardPageComponent = this.pageService.getNextPage(currentPage);
        ...
        if (!currentPage.completed) {
            ...
            this.pageService.commitPage(currentPage);
        }
    ...
    public previous(): void {
        ...
        previousPage = this.pageService.getPreviousPage(this.currentPage);
